...
def plot_validation_results(y_test, y_pred, fig_num=1):
    ...
    plt.title(f"Figure {fig_num}: Model Validation (Actual vs Predicted)\n$R^2 = {r2:.3f}$")
...
def plot_feature_importance(model, fig_num=2):
    ...
    plt.title(f"Figure {fig_num}: Feature Importance (Drivers of Health Burden)")
...
def run_stability_analysis(df, n_runs=50, fig_num=3):
    ...
    plt.title(f"Figure {fig_num}: Distribution of $R^2$ Score over {n_runs} Iterations")
